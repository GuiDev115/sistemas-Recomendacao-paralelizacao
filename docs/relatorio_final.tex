...
A paralelização com OpenMP é direta, utilizando-se a diretiva \texttt{parallel for}. Duas otimizações foram aplicadas:

\begin{enumerate}
\item \textbf{Escalonamento Dinâmico}: Devido ao loop triangular, as iterações têm cargas diferentes. O escalonamento dinâmico melhora o balanceamento.

\item \textbf{Collapse}: A diretiva \texttt{collapse(2)} permite que OpenMP paraleliza ambos os loops, aumentando o número de iterações disponíveis.
\end{enumerate}
...
O número de threads é controlado via variável de ambiente \texttt{OMP\_NUM\_THREADS}.
...
\begin{table*}[t]
\centering
\caption{Tempos de Execução (segundos) - Média de 5 execuções}
\label{tab:times}
\begin{tabular}{lcccccccc}
\toprule
\multirow{2}{*}{\textbf{Implementação}} & \multicolumn{4}{c}{\textbf{Dataset Large (1000×1000)}} & \multicolumn{4}{c}{\textbf{Dataset Medium (500×500)}} \\
\cmidrule(lr){2-5} \cmidrule(lr){6-9}
& \textbf{1 core} & \textbf{2 cores} & \textbf{4 cores} & \textbf{8 cores} & \textbf{1 core} & \textbf{2 cores} & \textbf{4 cores} & \textbf{8 cores} \\
\midrule
Sequencial & 124.3 & - & - & - & 15.8 & - & - & - \\
OpenMP & 124.5 & 63.2 & 32.1 & 20.1 & 15.9 & 8.1 & 4.2 & 2.6 \\
Pthreads & 124.8 & 64.5 & 33.8 & 21.5 & 15.9 & 8.4 & 4.5 & 2.8 \\
MPI & 125.2 & 67.8 & 38.5 & 27.8 & 16.1 & 9.2 & 5.3 & 3.6 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Implementação} & \textbf{1} & \textbf{2} & \textbf{4} & \textbf{8} \\
\midrule
Sequencial & 0.62 & - & - & - \\
OpenMP & 0.63 & 0.34 & 0.19 & 0.13 \\
Pthreads & 0.63 & 0.35 & 0.21 & 0.15 \\
MPI & 0.68 & 0.42 & 0.28 & 0.22 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Speedup Relativo à Versão Sequencial}
\label{tab:speedup}
\begin{tabular}{lcccc}
\toprule
\textbf{Implementação} & \textbf{2 cores} & \textbf{4 cores} & \textbf{8 cores} & \textbf{Dataset} \\
\midrule
OpenMP & 1.97 & 3.87 & 6.18 & Large \\
Pthreads & 1.93 & 3.68 & 5.78 & Large \\
MPI & 1.83 & 3.23 & 4.47 & Large \\
\midrule
OpenMP & 1.95 & 3.76 & 6.08 & Medium \\
Pthreads & 1.88 & 3.51 & 5.64 & Medium \\
MPI & 1.72 & 2.98 & 4.39 & Medium \\
\midrule
OpenMP & 1.82 & 3.26 & 4.77 & Small \\
Pthreads & 1.77 & 2.95 & 4.13 & Small \\
MPI & 1.48 & 2.21 & 2.82 & Small \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\caption{Speedup vs. Número de Cores - Dataset Large}
\label{fig:speedup}
\small
\begin{verbatim}
    7 |              ___OpenMP
...
\begin{tabular}{lccc}
\toprule
\textbf{Implementação} & \textbf{2 cores} & \textbf{4 cores} & \textbf{8 cores} \\
\midrule
OpenMP & 98.5 & 96.8 & 77.3 \\
Pthreads & 96.5 & 92.0 & 72.3 \\
MPI & 91.5 & 80.8 & 55.9 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{OpenMP} apresentou o melhor desempenho em todos os cenários, alcançando speedup de 6.18× com 8 cores no dataset Large. A eficiência manteve-se acima de 77\%, indicando baixo overhead e bom balanceamento de carga graças ao escalonamento dinâmico.

\textbf{Pthreads} obteve desempenho comparável a OpenMP, com speedup de 5.78×. A diferença de aproximadamente 7\% deve-se ao balanceamento imperfeito da partição estática, especialmente no loop triangular.

\textbf{MPI} apresentou speedup inferior (4.47×) devido ao overhead de comunicação. O broadcast da matriz de avaliações e a coleta de resultados via \texttt{MPI\_Reduce} introduzem latência significativa. Para o dataset Small, o overhead é proporcionalmente maior, resultando em speedup de apenas 2.82×.
...
\item \textbf{Datasets}: Uso de dados sintéticos. Dados reais apresentam sparsidade e distribuição diferentes.
...
\begin{enumerate}
\item OpenMP alcançou o melhor desempenho (speedup de 6.18× com 8 cores), combinando eficiência e simplicidade de implementação.

\item Pthreads apresentou desempenho competitivo (speedup de 5.78×) com maior flexibilidade, adequado para cenários que exigem controle fino.

\item MPI, apesar do speedup inferior em memória compartilhada (4.47×), mantém potencial de escalabilidade para ambientes distribuídos.

\item A eficiência paralela diminui com o aumento de cores, refletindo a Lei de Amdahl e o overhead de sincronização.

\item A escolha do paradigma deve considerar o trade-off entre desempenho, complexidade e escalabilidade requerida.
\end{enumerate}
